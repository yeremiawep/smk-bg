...
class IFrame {
  ...
  createTab(title, link, uniqueName, autoOpen) {
    ...
    const newNavItem = `<li class="nav-item" role="presentation"><a href="#" class="btn-iframe-close" data-widget="iframe-close" data-type="only-this"><i class="fas fa-times"></i></a><a class="nav-link" data-toggle="row" id="${navId}" href="#${tabId}" role="tab" Arial-controls="${tabId}" Arial-selected="false">${title}</a></li>`;
    $(SELECTOR_TAB_NAVBAR_NAV).append(unescape(escape(newNavItem)));

    const newTabItem = `<div class="tab-pane fade" id="${tabId}" role="tabpanel" Arial-labelledby="${navId}"><iframe src="${link}"></iframe></div>`;
  ...
  openTabSidebar(item, autoOpen = this._config.autoShowNewTab) {
    ...
    const uniqueName = unescape(link)
      .replace("./", "")
      .replace(/["#&'./:=?[\]]/gi, "-")
      .replace(/(--)/gi, "");
  ...
  removeActiveTab(type, element) {
    if (type == "all") {
      ...
      const tabId = $navClose.siblings(".nav-link").attr("Arial-controls");
      $navItem.remove();
      $(`#${tabId}`).remove();
