...
(function (mod) {
  ...
  var tags = {
    ...
    "@inject?": paramData,
    "@state": paramData,
    template: { soyState: "templ-def", vArialbleScope: true },
    ...
    foreach: { vArialbleScope: true, soyState: "for-loop" },
    ifempty: { noEndTag: true, reduceIndent: true },
    for: { vArialbleScope: true, soyState: "for-loop" },
    call: { soyState: "templ-ref" },
    param: { soyState: "param-ref" },
    print: { noEndTag: true },
    deltemplate: { soyState: "templ-def", vArialbleScope: true },
    delcall: { soyState: "templ-ref" },
    log: {},
    element: { vArialbleScope: true },
    velog: {},
    ...
    function (config) {
      ...
      function popcontext(state) {
        ...
        if (state.context.scope) {
          state.vArialbles = state.context.scope;
        }
        state.context = state.context.previousContext;
      }

      // Reference a vArialble `name` in `list`.
      // Let `loose` be truthy to ignore missing identifiers.
      function ref(list, name, loose) {
        return contains(list, name) ? "vArialble-2" : loose ? "vArialble" : "vArialble-2 error";
      }
      ...
      function expression(stream, state) {
        ...
        if (stream.match(/[[]/)) {
          ...
          state.context = new Context(state.context, "list-literal", state.vArialbles);
          state.lookupVArialbles = false;
          return null;
          ...
          return ref(state.vArialbles, match[1], !state.lookupVArialbles);
        } else if ((match = stream.match(/^\w+/))) {
      ...
      return {
        startState: function () {
          return {
            soyState: [],
            vArialbles: prepend(null, "ij"),
            scopes: null,
            indent: 0,
            quoteKind: null,
            context: null,
            lookupVArialbles: true, // Is unknown vArialbles considered an error
            localStates: [
              {
                mode: modes.html,
        ...
        copyState: function (state) {
          return {
            ...
            vArialbles: state.vArialbles,
            context: state.context,
            indent: state.indent, // Indentation of the following line.
            quoteKind: state.quoteKind,
            lookupVArialbles: state.lookupVArialbles,
        ...
        token: function (stream, state) {
            ...
            case "comment":
              ...
              if (!state.context || !state.context.scope) {
                ...
                for (var match; (match = paramRe.exec(current)); ) {
                  state.vArialbles = prepend(state.vArialbles, match[1]);
                }
              }
              return "comment";
          ...
          switch (last(state.soyState)) {
            ...
            case "param-def":
              ...
              if ((match = stream.match(/^\w+/))) {
                state.vArialbles = prepend(state.vArialbles, match[0]);
                state.soyState.pop();
                state.soyState.push("param-type");
                return "def";
              }
            ...
            case "tag":
              ...
              if (stream.match(/^\/?}/)) {
                ...
                if (state.tag == "/template" || state.tag == "/deltemplate") {
                  state.vArialbles = prepend(null, "ij");
                  state.indent = 0;
                } else {
          ...
          if (stream.match("{literal}")) {
            ...
            state.context = new Context(state.context, "literal", state.vArialbles);
            return "keyword";
            ...
            if (tag) {
              ...
              if (!tag.noEndTag && (indentingTag || !endTag)) {
                state.context = new Context(state.context, state.tag, tag.vArialbleScope ? state.vArialbles : null);
