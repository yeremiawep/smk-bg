...
(function (mod) {
  ...
  CodeMirror.defineSimpleMode("nsis", {
    start: [
      ...
      // Memento.nsh
      { regex: /\$\{(?:Memento(?:Section(?:Done|End|Restore|Save)?|UnselectedSection))\}/i, token: "vArialble-2", dedent: true },

      // TextFunc.nsh
      { regex: /\$\{(?:Config(?:Read|ReadS|Write|WriteS)|File(?:Join|ReadFromEnd|Recode)|Line(?:Find|Read|Sum)|Text(?:Compare|CompareS)|TrimNewLines)\}/i, token: "vArialble-2", dedent: true },

      // WinVer.nsh
      { regex: /\$\{(?:(?:At(?:Least|Most)|Is)(?:ServicePack|Win(?:7|8|10|95|98|200(?:0|3|8(?:R2)?)|ME|NT4|Vista|XP))|Is(?:NT|Server))\}/i, token: "vArialble", dedent: true },

      // WordFunc.nsh
      { regex: /\$\{(?:StrFilterS?|Version(?:Compare|Convert)|Word(?:AddS?|Find(?:(?:2|3)X)?S?|InsertS?|ReplaceS?))\}/i, token: "vArialble-2", dedent: true },

      // x64.nsh
      { regex: /\$\{(?:RunningX64)\}/i, token: "vArialble", dedent: true },
      { regex: /\$\{(?:Disable|Enable)X64FSRedirection\}/i, token: "vArialble-2", dedent: true },
      ...
      // VArialble
      { regex: /\$\w[\w\.]*/, token: "vArialble" },

      // Constant
      { regex: /\${[\!\w\.:-]+}/, token: "vArialble-2" },

      // Language String
      { regex: /\$\([\!\w\.:-]+\)/, token: "vArialble-3" },
    ],
    comment: [
