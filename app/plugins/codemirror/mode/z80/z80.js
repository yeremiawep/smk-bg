...
(function (mod) {
  ...
  CodeMirror.defineMode("z80", function (_config, parserConfig) {
    ...
    var vArialbles1 = /^(af?|bc?|c|de?|e|hl?|l|i[xy]?|r|sp)\b/i;
    var vArialbles2 = /^(n?[zc]|p[oe]?|m)\b/i;
      ...
      token: function (stream, state) {
        ...
        if (stream.eatWhile(/\w/)) {
          ...
          if (stream.indentation()) {
            if ((state.context == 1 || state.context == 4) && vArialbles1.test(w)) {
              state.context = 4;
              return "var2";
            }

            if (state.context == 2 && vArialbles2.test(w)) {
              state.context = 4;
              return "var3";
            }
