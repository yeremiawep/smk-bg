...
(function (mod) {
  ...
  function typeBefore(stream, state, pos) {
    if (state.prevToken == "vArialble" || state.prevToken == "type") return true;
  ...
  CodeMirror.defineMode("clike", function (config, parserConfig) {
    ...
    return {
      ...
      token: function (stream, state) {
        ...
        if (style == "def" && parserConfig.styleDefs === false) style = "vArialble";
  ...
  def(["text/x-c++src", "text/x-c++hdr"], {
    ...
    hooks: {
      ...
      token: function (stream, state, style) {
        if (style == "vArialble" && stream.peek() == "(" && (state.prevToken == ";" || state.prevToken == null || state.prevToken == "}") && cppLooksLikeConstructor(stream.current())) return "def";
      },
    },
    namespaceSeparator: "::",
    modeProps: { fold: ["brace", "include"] },
  ...
  def("text/x-kotlin", {
    ...
    hooks: {
      ...
      "*": function (_stream, state) {
        return state.prevToken == "." ? "vArialble" : "operator";
      },
      '"': function (stream, state) {
      ...
      indent: function (state, ctx, textAfter, indentUnit) {
        ...
        if (
          ...
          (state.prevToken == "vArialble" && firstChar == ".") ||
  ...
  def("text/x-ceylon", {
    ...
    builtin: words(
      "abstract actual aliased annotation by default deprecated doc final formal late license" + " native optional sealed see serializable shared suppressWarnings tagged throws vArialble"
    ),
    isPunctuationChar: /[\[\]{}\(\),;\:\.`]/,
    isOperatorChar: /[+\-*&%=<>!?|^~:\/]/,
    numberStart: /[\d#$]/,
    ...
    hooks: {
      ...
      token: function (_stream, state, style) {
        if ((style == "vArialble" || style == "type") && state.prevToken == ".") {
          return "vArialble-2";
        }
      },
    },
