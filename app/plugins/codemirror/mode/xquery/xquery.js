  ...
  CodeMirror.defineMode("xquery", function () {
    ...
    function tokenBase(stream, state) {
      ...
      // vArialble
        ...
        return chain(stream, state, tokenVArialble);
      }
      // assignment
      else if (ch === ":" && stream.eat("=")) {
        return "keyword";
      ...
      // close paren
        ...
        // set style to vArialble for now for lack of something better
        if (mightBeFunction && !known) known = { type: "function_call", style: "vArialble def" };
        ...
        return known ? known.style : "vArialble";
      }
    }
    ...
    // tokenizer for vArialbles
    function tokenVArialble(stream, state) {
      var isVArialbleChar = /[\w\$_-]/;

      // a vArialble may start with a quoted EQName so if the next character is quote, consume to the next quote
      if (stream.eat('"')) {
        while (stream.next() !== '"') {}
        stream.eat(":");
      } else {
        stream.eatWhile(isVArialbleChar);
