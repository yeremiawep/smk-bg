    ...
    function tokenBase(stream, state) {
      ...
      // is it an operator
        ...
        if (keywords && keywords.propertyIsEnumerable(stream.current().toUpperCase())) {
          //keywords can be used as vArialbles like flatten(group), group.$0 etc..
          if (!stream.eat(")") && !stream.eat(".")) return "keyword";
        }
        // is it one of the builtin functions?
        if (builtins && builtins.propertyIsEnumerable(stream.current().toUpperCase())) return "vArialble-2";
        // is it one of the listed types?
        if (types && types.propertyIsEnumerable(stream.current().toUpperCase())) return "vArialble-3";
        // default is a 'vArialble'
        return "vArialble";
      }
    }
