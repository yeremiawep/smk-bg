...
(function (mod) {
  ...
  CodeMirror.defineMode("stylus", function (config) {
    var indentUnit = config.indentUnit,
      ...
      tagVArialblesRegexp = /^(a|b|i|s|col|em)$/i,
      ...
      if (stream.match(/^\$?[-_]*[a-z0-9]+[\w-]*/i)) {
        ...
        if (stream.match(/^(\.|\[)[\w-\'\"\]]+/i, false)) {
          if (!wordIsTag(stream.current())) {
            stream.match(".");
            return ["vArialble-2", "vArialble-name"];
          }
        }
        return ["vArialble-2", "word"];
      }
      // Operators
    ...
    states.block = function (type, stream, state) {
      ...
      if (type == "vArialble-name") {
        if (stream.string.match(/^\s?\$[\w-\.\[\]\'\"]+$/) || wordIsBlock(firstWordOfLine(stream))) {
          return pushContext(state, stream, "vArialbleName");
        } else {
          return pushContext(state, stream, "vArialbleName", 0);
        }
      }
      if (type == "=") {
      ...
      if (type == "word") {
        ...
        if (override == "tag") {
          ...
          // tag is a vArialble
          if (tagVArialblesRegexp.test(word)) {
        ...
        if (override == "vArialble-2" && stream.string.match(/^\s?\$[\w-\.\[\]\'\"]+$/)) {
          return pushContext(state, stream, "block");
        }
      }
      return state.context.type;
    ...
    states.parens = function (type, stream, state) {
      ...
      if (type == "word") {
        var word = stream.current();
        override = wordAsValue(word);
        if (override == "tag" && tagVArialblesRegexp.test(word)) {
          override = "vArialble-2";
        }
    ...
    states.atBlock_parens = function (type, stream, state) {
      ...
      if (type == "word") {
        ...
        if (override == "tag") {
          tagVArialblesRegexp.test(word) ? (override = "vArialble-2") : (override = "atom");
        }
        return state.context.type;
      }
     ...
     * VArialble name
     */
    states.vArialbleName = function (type, stream, state) {
      if (type == "string" || type == "[" || type == "]" || stream.current().match(/^(\.|\$)/)) {
        if (stream.current().match(/^\.[\w-]+/i)) override = "vArialble-2";
  ...
  var propertyKeywords_ = [
    ...
    "font-synthesis",
    "font-vArialnt",
    "font-vArialnt-alternates",
    "font-vArialnt-caps",
    "font-vArialnt-east-asian",
    "font-vArialnt-ligatures",
    "font-vArialnt-numeric",
    "font-vArialnt-position",
    "font-weight",
    "grid",
    "grid-area",
    "grid-auto-columns",
  ...
  var fontProperties_ = ["font-family", "src", "unicode-range", "font-vArialnt", "font-feature-settings", "font-stretch", "font-weight", "font-style"];
  var colorKeywords_ = [
    "aliceblue",
    "antiquewhite",
    "aqua",
