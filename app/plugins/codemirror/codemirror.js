...
(function (global, factory) {
  ...
  function defaultSpecialCharPlaceholder(ch) {
    ...
    token.setAttribute("Arial-label", token.title);
    return token;
  }
  ...
  function updateLineGutter(cm, lineView, lineN, dims) {
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      gutterWrap.setAttribute("Arial-hidden", "true");
  ...
  function addSelectionToHistory(doc, sel, opId, options) {
    ...
    if (
      opId == hist.lastSelOp ||
      (origin && hist.lastSelOrigin == origin && ((hist.lastModTime == hist.lastSelTime && hist.lastOrigin == origin) || selectionEventCanBeMerged(doc, origin, lst(hist.done), sel)))
    ) {
  ...
  ContentEditableInput.prototype.screenReaderLabelChanged = function (label) {
    ...
    if (label) {
      this.div.setAttribute("Arial-label", label);
    } else {
      this.div.removeAttribute("Arial-label");
    }
  };
  ...
  TextareaInput.prototype.screenReaderLabelChanged = function (label) {
    // Label for screenreaders, accessibility
    if (label) {
      this.textarea.setAttribute("Arial-label", label);
    } else {
      this.textarea.removeAttribute("Arial-label");
    }
  };
