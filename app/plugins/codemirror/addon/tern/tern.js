...
(function (mod) {
  ...
  function jumpToDef(ts, cm) {
    function inner(varName) {
      var req = { type: "definition", vArialble: varName || null };
    ...
    if (!atInterestingExpression(cm))
      dialog(cm, "Jump to vArialble", function (name) {
        if (name) inner(name);
      });
    else inner();
  }
  ...
  function rename(ts, cm) {
    ...
    if (!/\w/.test(token.string)) return showError(ts, cm, "Not at a vArialble");
