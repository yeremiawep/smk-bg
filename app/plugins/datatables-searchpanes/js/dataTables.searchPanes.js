  ...
  var SearchPane = /** @class */ (function () {
    ...
    function SearchPane(paneSettings, opts, idx, layout, panesContainer, panes) {
      ...
      this.dom = {
        ...
        collapseButton: $('<button type="button"><span class="dtsp-caret">&#x5e;</span></button>').addClass(this.classes.paneButton).addClass(this.classes.collapseButton),
        container: $("<div/>")
        ...
        dtP: $(
          "<table><thead><tr><th>" +
            (this.colExists ? $(table.column(this.colExists ? this.s.index : 0).header()).text() : this.customPaneSettings.header || "Custom Pane") +
      ...
      this.dom.container.addClass(this.customPaneSettings !== null && this.customPaneSettings.className !== undefined ? this.customPaneSettings.className : "");
    ...
    SearchPane.prototype.getPaneCount = function () {
      return this.s.dtPane !== undefined ? this.s.dtPane.rows({ selected: true }).data().toArray().length : 0;
    };
    /**
    ...
    SearchPane.prototype.resize = function (layout) {
      ...
      this.dom.container
        ...
        .addClass(this.customPaneSettings !== null && this.customPaneSettings.className !== undefined ? this.customPaneSettings.className : "")
    ...
    SearchPane.prototype._buildPane = function (selectedRows, last, dataIn, init, prevEl) {
      ...
      if (this.colExists) {
        ...
        if (!this.s.dt.page.info().serverSide && (dataIn === null || dataIn.searchPanes === null || dataIn.searchPanes.options === null)) {
          ...
          // Don't show the pane if there isn't enough vArialnce in the data, or there is only 1 entry
          //  for that pane
          if (
            this.s.displayed === false &&
            ((colOpts.show === undefined && colOpts.threshold === null ? uniqueRatio > this.c.threshold : uniqueRatio > colOpts.threshold) || (colOpts.show !== true && binLength <= 1))
          ) {
            this.dom.container.addClass(this.classes.hidden);
            this.s.displayed = false;
            return;
          ...
          // Don't show the pane if there isnt enough vArialnce in the data, or there is only 1 entry for that pane
          if (
            this.s.displayed === false &&
            ((colOpts.show === undefined && colOpts.threshold === null ? uniqueRatio > this.c.threshold : uniqueRatio > colOpts.threshold) || (colOpts.show !== true && binLength <= 1))
          ) {
      ...
      // If the vArialnce is accceptable then display the search pane
      this._displayPane();
      if (!this.s.listSet) {
      ...
      this.s.dtPane = this.dom.dtP.DataTable(
        $.extend(
          ...
          this.s.colOpts.options !== undefined || !this.colExists
            ...
            : undefined,
          this.customPaneSettings !== null && this.customPaneSettings.dtOpts !== undefined ? this.customPaneSettings.dtOpts : {},
      ...
      if (this.colExists) {
        ...
        for (var i = 0, ien = rowData.arrayFilter.length; i < ien; i++) {
          ...
          if (
            ...
            (!this.c.cascadePanes || (this.c.cascadePanes && rowData.bins[rowData.arrayFilter[i].filter] !== 0) || (this.c.cascadePanes && init !== null) || selected)
          ) {
            var row = this.addRow(
              rowData.arrayFilter[i].display,
              rowData.arrayFilter[i].filter,
              init ? rowData.binsTotal[rowData.arrayFilter[i].filter] : rowData.bins[rowData.arrayFilter[i].filter],
              this.c.viewTotal || init ? String(rowData.binsTotal[rowData.arrayFilter[i].filter]) : rowData.bins[rowData.arrayFilter[i].filter],
    ...
    SearchPane.prototype._displayPane = function () {
      ...
      if (
        ...
        !this.c.controls ||
        !colOpts.controls ||
        (this.customPaneSettings !== null && this.customPaneSettings.dtOpts !== undefined && this.customPaneSettings.dtOpts.searching !== undefined && !this.customPaneSettings.dtOpts.searching)
      ) {
      ...
      if (((this.c.collapse && this.s.colOpts.collapse !== false) || this.s.colOpts.collapse) && this.c.controls && colOpts.controls) {
    ...
    SearchPane.prototype._populatePane = function (last) {
      ...
      if (!this.s.dt.page.info().serverSide) {
        var indexArray = (this.c.cascadePanes || this.c.viewTotal) && !this.s.clearing && !last ? table.rows({ search: "applied" }).indexes() : table.rows().indexes();
    ...
    SearchPane.prototype._updateCommon = function (draw) {
      ...
      if (
        ...
        if (this.colExists) {
          ...
          else if (this.c.cascadePanes && this.s.dt.rows().data().toArray().length === this.s.dt.rows({ search: "applied" }).data().toArray().length) {
          ...
          var _loop_1 = function (dataP) {
            ...
            if (dataP && ((rowData.bins[dataP.filter] !== undefined && rowData.bins[dataP.filter] !== 0 && this_1.c.cascadePanes) || !this_1.c.cascadePanes || this_1.s.clearing)) {
              var row = this_1.addRow(
                dataP.display,
                dataP.filter,
                !this_1.c.viewTotal ? rowData.bins[dataP.filter] : rowData.bins[dataP.filter] !== undefined ? rowData.bins[dataP.filter] : 0,
                this_1.c.viewTotal ? String(rowData.binsTotal[dataP.filter]) : rowData.bins[dataP.filter],
                dataP.sort,
                dataP.type
              );
  ...
  var SearchPanes = /** @class */ (function () {
    function SearchPanes(paneSettings, opts, fromInit) {
      ...
      this.dom = {
        ...
        showAll: $$1('<button type="button">Show All</button>').addClass(this.classes.showAll).addClass(this.classes.disabledButton).attr("disabled", "true"),
    ...
    SearchPanes.prototype.rebuild = function (targetIdx, maintainSelection) {
      ...
      for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
        ...
        returnArray.push(
          ...
          pane.rebuildPane(
            this.s.selectionList[this.s.selectionList.length - 1] !== undefined ? pane.s.index === this.s.selectionList[this.s.selectionList.length - 1].index : false,
    ...
    SearchPanes.prototype.redrawPanes = function (rebuild) {
      ...
      if (!this.s.updating && !this.s.dt.page.info().serverSide) {
        ...
        if (selectTotal === 0 && table.rows({ search: "applied" }).data().toArray().length === table.rows().data().toArray().length) {
          filterActive = false;
        }
      ...
      if (!selectPresent) {
        ...
        for (var i = 0; i < this.s.selectionList.length; i++) {
          ...
          if (!further) {
            ...
            for (var _b = 0, _c = this.s.panes; _b < _c.length; _b++) {
              var pane = _c[_b];
              if (pane.s.index === this.s.selectionList[i].index && pane.s.dtPane.rows({ selected: true }).data().toArray().length > 0) {
                push = true;
              }
            }
            if (push) {
        ...
        if (!_this.s.dt.page.info().serverSide) {
          _this.s.dt.one("preDraw", function () {
            ...
            for (var _i = 0, _a = _this.s.panes; _i < _a.length; _i++) {
              ...
              // when rebuilding
              pane.rebuildPane(
                _this.s.selectionList[_this.s.selectionList.length - 1] !== undefined ? pane.s.index === _this.s.selectionList[_this.s.selectionList.length - 1].index : false,
                undefined,
                undefined,
                true
              );
      ...
      for (var _h = 0, _j = this.s.panes; _h < _j.length; _h++) {
        ...
        if (
          ...
          ((pane.s.colOpts.preSelect !== undefined && pane.s.colOpts.preSelect.length > 0) ||
            (pane.customPaneSettings !== null && pane.customPaneSettings.preSelect !== undefined && pane.customPaneSettings.preSelect.length > 0))
        ) {
          var tableLength = pane.s.dtPane.rows().data().toArray().length;
          for (var i = 0; i < tableLength; i++) {
            if (
              pane.s.colOpts.preSelect.includes(pane.s.dtPane.cell(i, 0).data()) ||
              (pane.customPaneSettings !== null && pane.customPaneSettings.preSelect !== undefined && pane.customPaneSettings.preSelect.includes(pane.s.dtPane.cell(i, 0).data()))
            ) {
          ...
          $.extend(
            {
              filterChanged: function (count) {
                ...
                dt.button(node).text(
                  dt.i18n("searchPanes.collapse", dt.context[0].oLanguage.searchPanes !== undefined ? dt.context[0].oLanguage.searchPanes.collapse : dt.context[0]._searchPanes.c.i18n.collapse, count)
                );
              },
            },
            config.config
