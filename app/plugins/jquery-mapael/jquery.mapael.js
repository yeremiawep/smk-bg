...
(function (factory) {
  ...
  Mapael.prototype = {
        ...
        self.$map.on("touchstart." + pluginName, function (e) {
          if (e.originalEvent.touches.length === 2) {
            ...
            self.previousPinchDist = Math.sqrt(
              Math.pow(e.originalEvent.touches[1].pageX - e.originalEvent.touches[0].pageX, 2) + Math.pow(e.originalEvent.touches[1].pageY - e.originalEvent.touches[0].pageY, 2)
            );
          }
        });
        ...
        if (sliceOptions[i].display === undefined || sliceOptions[i].display === true) {
          if (legendType === "area") {
            ...
            if (sliceOptions[i].type === "image") {
              ...
              sliceOptions[i].attrs.transform =
                "m" + (scale * sliceOptions[i].width) / legendElemBBox.width + ",0,0," + (scale * sliceOptions[i].height) / legendElemBBox.height + "," + x + "," + y + sliceOptions[i].attrs.transform;
            }
          } else {
            if (legendOptions.mode === "horizontal") {
    ...
    getLegendSlice: function (value, legend) {
      for (var i = 0; i < legend.slices.length; ++i) {
        if (
          ...
          (legend.slices[i].sliceValue === undefined && (legend.slices[i].min === undefined || value >= legend.slices[i].min) && (legend.slices[i].max === undefined || value <= legend.slices[i].max))
        ) {
     ...
     *
     * This avoid using the internal vArialble paper._viewBox which
